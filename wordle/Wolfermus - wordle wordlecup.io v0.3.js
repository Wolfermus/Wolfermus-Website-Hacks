	...
	if(gameStatus == 1) {
			...
			if(currentRow == 0) {
				...
				if(Object.keys(gameData.lettersElsewhere).length > 0) {
					for(let elsewhereLetter in gameData.lettersElsewhere) {
						...
						let debugBox1 = document.createElement("p");
						debugBox1.innerHTML = `debugging 1: ${elsewhereLetter}`;
						document.getElementById("root").prepend(debugBox1);
						...
						for(let wordPos in tempCopy) {
							...
							let debugBox2 = document.createElement("p");
							debugBox2.innerHTML = `debugging 2: ${word}`;
							document.getElementById("root").prepend(debugBox2);
							
							for(let letterPos in word) {
								let debug3Box1 = document.createElement("p");
								debug3Box1.innerHTML = `debugging 3.1: ${letterPos} - ${word[letterPos]}`;
								document.getElementById("root").prepend(debug3Box1);
							
								if(!Object.keys(gameData.lettersCorrect).includes(letterPos) && word[letterPos] == elsewhereLetter) {
									let debug3Box1o1 = document.createElement("p");
									debug3Box1o1.innerHTML = `debugging 3.1.1: ${elsewhereLetter}`;
									document.getElementById("root").prepend(debug3Box1o1);
									
									let debug3Box1o2 = document.createElement("p");
									debug3Box1o2.innerHTML = `debugging 3.1.2: ${letterPos}`;
									document.getElementById("root").prepend(debug3Box1o2);
									
									let debug3Box1o3 = document.createElement("p");
									debug3Box1o3.innerHTML = `debugging 3.1.3: ${gameData.lettersElsewhere[elsewhereLetter].pos}`;
									document.getElementById("root").prepend(debug3Box1o3);
									
									let debug3Box1o4 = document.createElement("p");
									debug3Box1o4.innerHTML = `debugging 3.1.4: ${gameData.lettersElsewhere[elsewhereLetter].pos.includes(letterPos)}`;
									document.getElementById("root").prepend(debug3Box1o4);
									
									if(gameData.lettersElsewhere[elsewhereLetter].pos.includes(letterPos)) {
										await containsLetterInPos.push(elsewhereLetter);
										
										let debug3Box2 = document.createElement("p");
										debug3Box2.innerHTML = `debugging 3.2: containsLetterInPos - ${elsewhereLetter}`;
										document.getElementById("root").prepend(debug3Box2);
									} else {
										await containsLetter.push(elsewhereLetter);
										
										let debug3Box3 = document.createElement("p");
										debug3Box3.innerHTML = `debugging 3.3: containsLetter - ${elsewhereLetter}`;
										document.getElementById("root").prepend(debug3Box3);
									}
								}
							}
							
							if(containsLetterInPos.length > 0 || containsLetter.length < gameData.lettersElsewhere[elsewhereLetter].num) {
								await gameData.words.splice(gameData.words.indexOf(word), 1)
								
								let debugBox4 = document.createElement("p");
								debugBox4.innerHTML = `debugging 4: splice - ${word}`;
								document.getElementById("root").prepend(debugBox4);
							}
						}
					}
				}
		...
		for (let i = 0; i < row.length; i++) {
			...
			if(rowClassList.contains("letter-correct")) {
				...
				await letterElsewhere[rowLetter.textContent].push(i);
